...
export function parse(code: string) {
  ...
  const structs = readExpressions(shaderCode, { blocks, expressionFilter: expressionStructsFilter });
  ...
  const extraTypes = structs.map((s) => s.name).filter((s) => typeof s === 'string') as string[];
  ...
  return readExpressions(shaderCode, { blocks, extraTypes }).concat(structs).map(
...
function isGLSLVariable(value: unknown): value is GLSLVariable {
  return (
    ...
    ((value as GLSLVariable).qualifier === 'struct' || isQualifier((value as GLSLVariable).qualifier)) &&
    ...
    'layout' in value &&
    ((value as GLSLVariable).layout === null || typeof (value as GLSLVariable).layout === 'string') &&
    // the "precision" attribute must defined and be either null or a valid
    // precision string
    'precision' in value &&
    ((value as GLSLVariable).precision === null || isGLSLPrecision((value as GLSLVariable).precision as string))
  );
}
 ...
 **/
export function isInputVariable(variable: GLSLVariable): variable is InputGLSLVariable {
 ...
 **/
export function isOutputVariable(variable: GLSLVariable): variable is OutputGLSLVariable {
  return variable.qualifier === 'out';
}
 ...
 **/
export function isSamplerVariable(variable: GLSLVariable): variable is SamplerGLSLVariable {
